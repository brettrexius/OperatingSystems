...
int main() {
	cout << "Sender, PID " << getpid() << ", begins execution." << endl; // Sender announces its execution.  PID was created at spawning process

	int qid = msgget(ftok(".", 'u'), 0); // Same path and key as Master so it will be sending the message on the same message queue. 0 means that it will not be creating a message queue.

	buf msg; // declaring a message type buf

	string line; // declares a string for user to input
	cout << "Sender, PID " << getpid() << ", would like you to send a message: "; // Sender asks user for a message
	getline(cin, line); // User inputs a message

	strcpy(msg.greeting, line.c_str()); // converts user input string to a char array and copies it to greeting
	...
	msgsnd(qid, (struct msgbuf *)&msg, size, 0); // sends the message from the user to the message queue from strcpy function, to be received by Receiver
												// sends a message on the message queue of qid
												// of size msg - mtype
												// msgflg = 0 means execution will wait until the other process is finished
	
	cout << "Sender, PID " << getpid() << ", sends message: " << msg.greeting << endl; // Sender announces that it has sent the user's message to the message queue and outputs the message
									
	cout << "Sender, PID " << getpid() << ", now exits." << endl; // Sender announces it exits											

	exit(0); // Sender terminates successfully
}